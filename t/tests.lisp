(defpackage #:bip0340/test
  (:use #:cl
	#:bip0340
	#:fiveam))

(in-package #:bip0340/test)
(def-suite :bip0340)
(in-suite :bip0340)
...
(test on-bip340-test-vectors
  (dolist (tv-row *test-vectors*)
    (destructuring-bind (num secret-key public-key aux-rand message signature verification comment) tv-row
      (declare (ignorable num comment))
      (let ((secret-key (crypto:hex-string-to-byte-array secret-key))
	    (public-key (crypto:hex-string-to-byte-array public-key))
	    (aux-rand (crypto:hex-string-to-byte-array aux-rand))
	    (message (crypto:hex-string-to-byte-array message))
	    (signature (crypto:hex-string-to-byte-array signature))
	    (verification (cond ((string= "FALSE" verification) nil)
				((string= "TRUE" verification) t)
				(t (error "FALSE or TRUE")))))
	(if (plusp (length secret-key))
	    (let ((signature* (bip0340:sign-message secret-key message aux-rand)))
	      (is (equalp signature* signature)))
	    (is (equal verification (verify-signature public-key message signature))))))))
